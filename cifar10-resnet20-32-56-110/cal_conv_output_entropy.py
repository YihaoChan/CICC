...
def test(args, network, test_data_loader, batch_limit):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_data_loader):
            ...
            network(inputs)  # forward propagation to activate the hooks to append the entropy of feature maps
...
def calculate(args, network, test_data_loader):
        ...
        if isinstance(module, BasicBlock):
            for m in module.children():
                ...
                if type(m) == nn.BatchNorm2d:
                    conv_module_num += 1
                    m.register_forward_hook(hook_conv_output_entropy)  # inject the hook
    ...
    for limit in batch_limit:
        ...
        cal_entropy(args, mean_conv_outputs, limit)
        conv_outputs.clear()


def cal_entropy(args, items, batch_num):
    res = []

    for tensor in tqdm(items):
        # Only with ReLU can the entropys not be full, cuz ReLU makes the negative weights be zero.
    ...
    save_dir = os.path.join('trained_entropy_npy/%s/batches_%d' % (args.network, batch_num))
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    np.save(os.path.join(save_dir, '%s_entropy.npy') % args.network, res)

    log.logger.info("=" * 10 + "Entropy of full-trained %s under batch %d" % (args.network, batch_num) + "=" * 10)
