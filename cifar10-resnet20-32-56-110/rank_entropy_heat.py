...
def load():
    ...
    parser.add_argument('--path', type=str, default='D:\\programme\\BMVC\\images\\rank_entropy\\heat')
...
def stat(args):
    ...
    for root, dirs, files in os.walk(os.path.join('./trained_%s_npy/%s' % (args.type, args.network))):
        for file in files:
...
def plot_rank_entropy(feature_maps_entropy, batches_num, args):
    ...
    save_fig_path = os.path.join(args.path, '%s_%s_heat.eps' % (args.network, args.type))
