...
def resnet_forward_partial(self, x, to_module=None, from_module=None):
    processing = from_module is None

    layers = torch.nn.Sequential(*(list(self.children())[:-1])).children()
...
def get_dataset_and_loaders(
    ...
    train_set, test_set = get_dataset(args=args)
