...
def train_network(args, network=None):
    if args.train_flag:
        ...
        network = VGG16()  # network为None，说明是第一次传参，即为train阶段

        log.logger.info("=" * 10 + "Train network %s" % args.network + "=" * 10)
    elif args.resume_prune_flag:
    ...
    for epoch in range(args.start_epoch, args.epoch):
        ...
        model_save_path = ''
        if args.train_flag:
            model_save_path = os.path.join(args.train_save_path, "%s_check_point.pth" % args.network)
        elif args.resume_prune_flag:
