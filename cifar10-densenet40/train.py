...
def train_network(args, network=None):
    ...
    if args.train_flag:
        ...
        log.logger.info("=" * 10 + "Train network DenseNet40" + "=" * 10)
    elif args.prune_flag:
        assert network is not None
        save_path = args.prune_save_path

        log.logger.info("=" * 10 + "Retrain network DenseNet40" + "=" * 10)
    ...
    for epoch in range(args.start_epoch, args.epoch):
        ...
        model_save_path = os.path.join(save_path, "densenet40_check_point.pth")
        if args.multi_gpu:
