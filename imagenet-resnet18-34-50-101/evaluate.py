...
def test_network(args, network=None):
    if network is None:  # 只推理，没有上游传下来的模型
        ...
        if args.resume_prune_flag:
            network = torch.load(os.path.join(args.resume_save_path, "%s_check_point.pth" % args.network),
                                 map_location='cuda:%s' % args.device_ids[0])
...
def test_step(args, network, test_data_loader):
    ...
    log.logger.info("=" * 10 + "Evaluate network %s" % args.network + "=" * 10)

    with torch.no_grad():
        for iteration, (inputs, targets) in enumerate(test_data_loader):
            ...
            if iteration % 10 == 0:
                ...
                str_ += 'Iteration [%d/%d]' % (iteration, len(test_data_loader))
